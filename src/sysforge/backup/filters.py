...
class FileFilter:
    ...
    def _should_include_git_file(
        self, file_path: Path, git_repo: GitRepository
    ) -> tuple[bool, str]:
    ...
    def should_include_directory(self, dir_path: Path) -> tuple[bool, str]:
        ...
        if git_repo:
            ...
            if self.config.git.respect_gitignore and git_repo.is_ignored(dir_path):
                ...
                if self._matches_patterns(
                    dir_path, self.config.git.gitignore_override_patterns
                ):
    ...
    def get_filtered_files(
        ...
        if verbose and console:
            console.print(
                "[dim]Using high-performance find-based file discovery...[/dim]"
            )
        ...
        if self.config.enable_parallel_processing:
            file_paths = self._get_non_repository_files_parallel(
                base_path, git_repos, verbose, console
            )
        else:
            file_paths = self._get_non_repository_files_sequential(
                base_path, git_repos, verbose, console
            )
        ...
        for file_path in file_paths:
            ...
            if not self._check_file_size(file_path):
                if verbose and console:
                    console.print(
                        f"[dim]Excluded: {file_path} (size exceeds limit)[/dim]"
                    )
                continue
        ...
        if git_repos and self.config.git.include_repos:
            if self.config.enable_parallel_processing:
                git_files_filtered = self._get_repository_files_parallel(
                    git_repos, verbose, console
                )
            else:
                git_files_filtered = self._get_repository_files_sequential(
                    git_repos, verbose, console
                )
        ...
        if verbose and console:
            console.print("[dim]Find-based scan complete:[/dim]")
            console.print(
    ...
    def _scan_home_directory_focused(
        ...
        if verbose and console:
            console.print(
                f"[dim]Scanning {len(subdirs)} important subdirectories...[/dim]"
            )
    ...
    def _discover_git_repositories_parallel(
        ...
        try:
            ...
            path_chunks = [
                search_paths[i : i + 3] for i in range(0, len(search_paths), 3)
            ]

            with ThreadPoolExecutor(
                max_workers=min(self.config.max_workers, len(path_chunks))
            ) as executor:
                futures = [
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        if self.verbose and self.console:
                            self.console.print(
                                f"[red]Error in parallel git discovery: {e}[/red]"
                            )
        ...
        with ThreadPoolExecutor(
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if verbose and console:
                        console.print(
                            f"[red]Error processing repo {repo.path}: {e}[/red]"
                        )
        ...
        for repo in repos:
            if self.config.git.respect_gitignore:
                ...
                if self.config.git.gitignore_override_patterns:
                    override_files = repo.get_override_files(
                        self.config.git.gitignore_override_patterns
                    )
                ...
                if verbose and console:
                    console.print(
                        f"[dim]Adding complete git repository: {repo.path}[/dim]"
                    )

                # Get all repository files including .git directory
                repo_files = repo.get_all_repo_files(
                    include_git_dir=self.config.git.include_git_dir
                )
    ...
    def _process_single_repository(self, repo: GitRepository) -> List[Path]:
        ...
        if self.config.git.respect_gitignore:
            ...
            filtered_files = repo.get_all_repo_files(
                include_git_dir=self.config.git.include_git_dir
            )
    ...
    def _get_non_repository_files_parallel(
        ...
        with ThreadPoolExecutor(
            max_workers=min(self.config.max_workers, len(search_chunks))
        ) as executor:
            futures = [
        ...
        if base_path == Path.home():
            ...
            try:
                ...
                if result.returncode != 0:
                    if verbose and console:
                        console.print(
                            f"[red]Find command failed: {result.stderr}[/red]"
                        )
            ...
            if "**" in pattern:
                for relative_path in relative_path_variations:
                    ...
                    if pattern.startswith("**/") and pattern.endswith("/**"):
                        ...
                        if (
                            f"/{middle}/" in relative_path
                            or relative_path.startswith(f"{middle}/")
                            or relative_path.endswith(f"/{middle}")
                            or relative_path == middle
                        ):
                            return True
                        ...
                        if (
                            ...
                            or fnmatch.fnmatch(relative_path, suffix)
                            or ("/" + suffix) in relative_path
                        ):
                            return True
