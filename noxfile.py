...
shutil.rmtree('src/sysforge.egg-info', ignore_errors=True);
for f in glob.glob('src/sysforge/*.so'): os.remove(f);
for f in glob.glob('src/sysforge/*.c'): os.remove(f);
"""
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"SYSFORGE_COMPILE_MYPYC": "1"})
