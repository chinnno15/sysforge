...
class TestTargetConfig:
    ...
    def test_default_target_config(self) -> None:
...
class TestGitConfig:
    ...
    def test_default_git_config(self) -> None:
        ...
        assert (
            config.respect_gitignore is True
        )  # Respect gitignore by default for sensible backup sizes
        assert config.include_git_dir is True
        assert config.backup_complete_git is True  # New field for complete git backup
        assert (
            len(config.gitignore_override_patterns) > 0
        )  # Should have default override patterns
...
class TestRestoreConfig:
    ...
    def test_default_restore_config(self) -> None:
    ...
    def test_get_backup_suffix(self) -> None:
...
class TestBackupConfig:
    ...
    def test_default_backup_config(self) -> None:
    ...
    def test_get_max_file_size_bytes(self) -> None:
...
class TestConfigManager:
    ...
    def test_merge_configs(self) -> None:
        ...
        config1 = {"compression": {"level": 5}, "target": {"base_path": "~/Work"}}

        config2 = {"compression": {"format": "lz4"}, "git": {"include_repos": False}}
    ...
    def test_merge_configs_with_none(self) -> None:
    ...
    @patch("builtins.open")
    def test_load_user_config(self, mock_open, mock_exists) -> None:
    ...
    @patch("sysforge.backup.config.Path.exists")
    def test_load_user_config_not_exists(self, mock_exists) -> None:
        """Test loading user config when file doesn't exist."""
        mock_exists.return_value = False
        config = ConfigManager.load_user_config()
        assert config is None

    def test_load_effective_config_default_only(self) -> None:
    ...
    def test_load_effective_config_with_overrides(self) -> None:
    ...
    def test_save_and_load_config_integration(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(ConfigManager, "CONFIG_DIR", temp_path):
                with patch.object(
                    ConfigManager, "USER_CONFIG_FILE", temp_path / "config.yaml"
                ):
                    # Save config
                    test_config = {
                        "compression": {"level": 7},
